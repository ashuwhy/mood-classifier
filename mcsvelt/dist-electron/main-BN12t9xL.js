...
async function downloadWithYtDlp(url) {
  if (!youtubeDlExec) {
    try {
      ...
      const possibleBinPaths = [
        ...
        path.resolve(path.join(path.dirname(fileURLToPath(import.meta.url)), "bin", binName)),
      ...
      for (const testPath of possibleBinPaths) {
        console.log("Checking for yt-dlp at:", testPath);
      ...
      if (binPath) {
        console.log("Using custom binary path for youtube-dl-exec:", binPath);
        youtubeDlExec = ytdlModule.create(binPath);
      } else {
        console.log("Binary not found at any expected path, using default");
  ...
  if (!ffmpeg) {
    try {
      const ffmpegImport = await import("./index-Cx2GWZLa.js").then((n) => n.i);
      ffmpeg = ffmpegImport.default;
      try {
        const require2 = createRequire(import.meta.url);
        const ffmpegStatic = require2("ffmpeg-static");
  ...
  return new Promise((resolve, reject) => {
    try {
      const tempFilePath = path.resolve(tempFile);
      const outputFilePath = path.resolve(String(filePath));
      console.log("Converting from:", tempFilePath);
      ...
      command.outputOptions([
        ...
        if (progress && progress.percent) {
          console.log("FFmpeg processing progress:", progress.percent.toFixed(1), "%");
        }
      }).on("end", () => {
      ...
      console.error("Error during ffmpeg setup:", error);
      reject(error);
    }
  });
}
...
//# sourceMappingURL=main-BN12t9xL.js.map
