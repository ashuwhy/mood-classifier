...
function requireMain() {
  ...
  function _resolveHome(envPath) {
    return envPath[0] === "~" ? path$1.join(os2.homedir(), envPath.slice(1)) : envPath;
  }
...
ipcMain.handle("recognize-music", async (event, filePath) => {
  try {
    ...
    if (stat.size > MAX_SIZE) {
      ...
      const ffmpegStaticPath = require2("ffmpeg-static");
      return new Promise((resolve, reject) => {
        const args = [
          "-i",
          filePath,
        ...
        ffmpegProcess.on("close", async (code) => {
          if (code === 0) {
            ...
            try {
              const result2 = await recognizeMusicWithACRCloud(tempFilePath);
...
ipcMain.handle("fetch-image", async (event, imageUrl) => {
  try {
    ...
    if (fs.existsSync(tempFilePath)) {
      ...
      const imageBuffer2 = fs.readFileSync(tempFilePath);
...
console.log("Registered IPC handler for: cleanup-temp-files");
...
async function downloadWithYtDlp(url) {
  if (!youtubeDlExec) {
    try {
      const require2 = createRequire(import.meta.url);
      const ytdlModule = require2("youtube-dl-exec");
  ...
  try {
    const require2 = createRequire(import.meta.url);
    ffmpegStaticPath = require2("ffmpeg-static");
  ...
  const videoInfo = await youtubeDlExec(url, {
    ...
    youtubeDlpPath: ytdlpBinaryPath
      ...
      if (code === 0) {
        ...
        resolve({
          ...
          videoDetails: {
            ...
            // Make sure videoInfo is still accessible here
          }
        });
      } else {
...
async function recognizeMusicWithACRCloud(filePath) {
  try {
    const require2 = createRequire(import.meta.url);
    ...
    console.log(
      "ACRCloud API response received:",
    ...
    if (response.data.status.code === 0 && response.data.metadata && response.data.metadata.music && response.data.metadata.music.length > 0) {
      ...
      if (music.album && music.album.images) {
        ...
        if (music.external_metadata.spotify && music.external_metadata.spotify.album && music.external_metadata.spotify.album.images) {
        ...
        } else if (music.external_metadata.deezer && music.external_metadata.deezer.album && music.external_metadata.deezer.album.cover) {
...
function cleanupTempFiles() {
  try {
    ...
    const tempFilePatterns = [
      ...
      // Uploaded temp files
      /^temp_short_\d+\.wav$/,
      // Shortened audio files
      /^\d+\.m4a$/
      // YouTube-dl temporary files
    ];
    for (const file of files) {
      ...
      if (isTemp) {
        ...
        try {
          ...
          deletedCount++;
          console.log(`Deleted temp file: ${file} (${fileSizeKB}KB, ${Math.round(fileAge / 1e3 / 60)} minutes old)`);
        } catch (err) {
...
function registerCleanupHandlers() {
  ...
  const CLEANUP_INTERVAL = 30 * 60 * 1e3;
  setInterval(() => {
