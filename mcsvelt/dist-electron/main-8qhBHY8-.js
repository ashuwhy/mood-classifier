...
async function optimizeStartup() {
  ...
  ipcMain.handle("read-audio-file", async (event, filePath) => {
    try {
      ...
      const buffer = await require$$0.promises.readFile(filePath);
      return buffer;
    } catch (error) {
...
async function downloadWithYtDlp(url) {
  if (!youtubeDlExec) {
    try {
      youtubeDlExec = require2("youtube-dl-exec");
    } catch (error) {
      console.error("Failed to load youtube-dl-exec:", error);
      throw new Error(`Failed to require youtube-dl-exec. Ensure it's installed and compatible. Original error: ${error.message}`);
    }
  }
  if (!ffmpeg) {
    try {
      const ffmpegImport = await import("./index-DOTPTRu7.js").then((n) => n.i);
      ffmpeg = ffmpegImport.default;
      try {
        const ffmpegStaticImport = await import("./index-CbrnqzQM.js").then((n) => n.i);
  ...
  const tempFile = path.join(__dirname, `${Date.now()}.m4a`);
  ...
  if (!require$$0.existsSync(tempFile)) {
...
//# sourceMappingURL=main-8qhBHY8-.js.map
